...
import { Controller, MapFormat, ILocation, PixelRef } from '../map';
...
class State {
  ...
  pixel(addr: string, ref?: PixelRef) {
...
function queue(groups: number[][], then: Action) {
  // Keyless map: every location comes from coordinates (the lat/long columns or manual
  // relocate), so there is no geocoding step — lay out each group directly. Addresses
  // without coordinates are marked "unlocated" inside addGroup and simply not drawn.
