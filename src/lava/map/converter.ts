import type { IMapShim } from './controller';
import { IPoint, StringMap, clamp } from '../type';
...
// Fitting the view to the data is now done natively by Leaflet's fitBounds in the
// controller, so the former Bing-specific fitOptions() was removed.

export function anchorPixel(m: IMapShim, bound: IBound): IPoint {
    let level = m.getZoom(), { anchor, margin } = bound;
    let loc = { latitude: anchor.latitude, longitude: anchor.longitude };
