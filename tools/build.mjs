/**
 * Build helper for the Flow map visual.
 *
 * On each run it:
...
/** Read a JSON file, returning both the parsed value and a writer that preserves indentation. */
function readJson(file, indent) {
    const value = JSON.parse(fs.readFileSync(file, "utf8"));
    const write = (next) => fs.writeFileSync(file, JSON.stringify(next, null, indent) + "\n", "utf8");
    return { value, write };
}

/**
 * Bump the last (build) segment of the 4-part version, e.g. 1.4.9.0 -> 1.4.9.1
 * (Power BI requires a 4-part version). Major/minor/patch bumps are done by hand in
 * pbiviz.json / package.json.
 */
...
const pbiviz = readJson(pbivizPath, 4);
const pkg = readJson(packagePath, 2);
